...
class ListRepoViewModel(private val schedulerProvider: SchedulerProvider, private val apiHelper: ApiHelper) : ViewModel() {
    ...
    init {
        compositeDisposable.add(RxBinding.toObservable(email)
                ...
                // hit api only if the name entered is different from the previous entry
